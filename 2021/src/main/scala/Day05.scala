package it.imolab.adventOfCode._2021.Day05
import scala.io.Source

private val puzzle: List[String] = Source.fromFile("2021/src/main/resources/Day05.input.txt").getLines.toList
...
The top-left pair of 1s, for example, comes from 2,2 -> 2,1; the very bottom row is formed by the
...
def solve_1 (p: List[String]): Integer =
  p.map(parseSegment).filter(isOrthogonal)
    .flatMap(segmentPoints)
    .groupBy(identity)
    .filter((p, pp) => pp.size > 1)
    .values.size
...
@main def answer_1 = println("2021 - Day 05 - answer 1: " + solve_1(puzzle))

@main def answer_2 = println("2021 - Day 05 - answer 2: " + solve_2(puzzle))

// #######################################################################

private case class Point(x:Int, y:Int)
private case class Segment(p1: Point, p2: Point)

private def parseSegment (l: String) : Segment =
  val pattern = """(\d+),(\d+) -> (\d+),(\d+)""".r
  val m = pattern.findAllIn(l).matchData.next
  Segment(Point(m.group(1).toInt, m.group(2).toInt), Point(m.group(3).toInt, m.group(4).toInt))

private def isOrthogonal (s: Segment) : Boolean =
  s.p1.x == s.p2.x || s.p1.y == s.p2.y
  
private def segmentPoints (s: Segment) : List[Point] =
  // val x1 = Math.min(s.p1.x, s.p2.x)
  // val x2 = Math.max(s.p1.x, s.p2.x)

  // val y1 = Math.min(s.p1.y, s.p2.y)
  // val y2 = Math.max(s.p1.y, s.p2.y)

  val xs = Range.inclusive(s.p1.x, s.p2.x, if s.p1.x < s.p2.x then 1 else -1)
  val ys = Range.inclusive(s.p1.y, s.p2.y, if s.p1.y < s.p2.y then 1 else -1)

  if (s.p1.x == s.p2.x) || (s.p1.y == s.p2.y) then
    val points = for
      x <- xs
      y <- ys
    yield Point(x, y)
    ...
    xs.zip(ys).map((x, y) => Point(x,y)).toList
